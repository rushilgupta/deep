...
with graph.as_default():
  ...
  # normal distribution (mean = 0 and equal variances). The biases get initialized to zero.
  weights = tf.Variable(
...
with graph.as_default():
  ...
  # First NN \m/
  def three_layer_network(tf_train_dataset):
      # W1X+b
  ...
  logits = three_layer_network(tf_train_dataset)
  ...
  train_prediction = tf.nn.softmax(logits)
  valid_prediction = tf.nn.softmax(
    three_layer_network(tf_valid_dataset))
